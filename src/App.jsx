...
export default function App() {
  ...
  /* useEffect examples
    ...
    useEffect(() => {
      console.log('After every render in "useEffect"');
    });
  
    useEffect(() => {
      console.log('After query re-render')
    }, [query]);
  
    console.log('After every render in "App"');
  ...
  function handleDeleteWatchedMovie(id) {
    setWatched(watched.filter((watchedObj) => watchedObj.imdbID !== id));

    /* localStorage.setItem('watched', JSON.stringify(watched.filter((watchedObj) => watchedObj.imdbID !== id))); */
  }
...
function MovieDetails({ selectedMovieId, onCloseMovie, onAddWatchedMovie, watched }) {
  ...
  // Another Solution
  /* const indexOfWatchedMovie = watched.map((movie) => movie.imdbID).indexOf(selectedMovieId);
  const watchedUserRating = indexOfWatchedMovie >= 0 ? watched[indexOfWatchedMovie].userRating : 0; */
  ...
  function handleAdd() {
    const newWatchedMovie = {
      ...
      userRating,
      decisionMakingUserRating: countRef.current,
    };
  ...
  useEffect(() => {
    ...
    async function getMovieDetails() {
      try {
        ...
        if (!res.ok) {
          throw new Error('there is something wrong in fetch');
        }
        ...
        <>
          ...
          <section>
            <div className='rating'>
              {isWatched ? (
                <p>You have rated this movie before with {watchedUserRating} ⭐</p>
              ) : (
                <>
